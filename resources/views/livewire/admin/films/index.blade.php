<div class="h-full overflow-y-auto scrollbar-hide">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 animate-fadeIn">
            <div>
                <h1 class="text-4xl font-black text-white uppercase tracking-tighter logo">
                    CINEWATCH <span class="text-[#E50914]">ENGINE</span>
                </h1>
                <div class="flex items-center gap-3 mt-1">
                    <p class="text-gray-500 text-[10px] font-bold uppercase tracking-[3px] opacity-60">Pusat Distribusi & Pengelolaan Konten Digital</p>
                    @if($genreFilter || $search)
            ...
            <div class="flex items-center gap-4 flex-1 max-w-2xl">
                <div class="relative flex-1 group">
                    ...
                    <input 
                        ...
                        class="w-full bg-white/[0.03] border border-white/5 rounded-2xl pl-12 pr-6 py-4 text-sm font-bold text-white focus:border-red-600 outline-none transition-all shadow-2xl"
                    />
                </div>
                <a href="{{ route('admin.films.create') }}" class="bg-[#E50914] text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-[2px] shadow-xl shadow-red-900/40 hover:scale-[1.02] active:scale-95 transition-all flex items-center gap-3 shrink-0">
                    <span class="material-symbols-outlined text-[18px]">add_circle</span> TAMBAH KARYA
                </a>
            </div>
        </div>
        ...
        <div class="bg-neutral-900/40 backdrop-blur-3xl border border-white/5 rounded-2xl overflow-hidden shadow-2xl">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="text-sm text-gray-300">
                    ...
                    <tr class="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors group">
                        ...
                        <td class="px-8 py-5">
                            @if($film->audience_type === 'kids')
                                <span class="bg-yellow-500/10 border border-yellow-500/20 text-yellow-600 px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest flex items-center gap-1.5 w-fit">
                                    <span class="material-symbols-outlined text-[12px]">child_care</span> KIDS
                                </span>
                            @else
                                <span class="bg-red-600/10 border border-red-600/20 text-red-600 px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest flex items-center gap-1.5 w-fit">
                        ...
                        <td class="px-8 py-5">
                            <div class="flex items-center justify-end gap-3">
                                <a href="{{ route('admin.films.edit', $film->id) }}" class="w-10 h-10 bg-blue-500/10 text-blue-500 rounded-lg flex items-center justify-center hover:bg-blue-500 hover:text-white transition-all">
                    ...
                    @empty
                    <tr>
                        <td colspan="4" class="px-8 py-20 text-center text-gray-700 italic uppercase font-black text-xs tracking-[5px]">
                            <span class="material-symbols-outlined text-5xl mb-4 block">folder_off</span> Arsip Database Kosong
                        </td>
                    </tr>
                    @endforelse
                </tbody>
    ...
    <div class="fixed inset-0 z-[1000] flex items-center justify-center p-6 bg-black/80 backdrop-blur-md animate-fadeIn">
        <div class="w-full max-w-md bg-neutral-900 border border-white/10 rounded-3xl p-10 text-center shadow-[0_0_100px_rgba(229,9,20,0.3)] animate-cardSlide">
            ...
            <p class="text-gray-500 text-[11px] font-bold uppercase tracking-widest leading-relaxed mb-10 italic">"Tindakan ini akan menghapus data film secara permanen dari terminal arsip. Operasi ini tidak dapat dibatalkan."</p>
